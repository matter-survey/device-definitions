...
export const allDevices: DeviceDefinition[] = [...eveDevices, ...aqaraDevices];
...
export function findDevice(
  ...
  return allDevices.find((device) => {
    ...
    if (fp.requiredClusters && clusterIds) {
      const hasAllClusters = fp.requiredClusters.every((id) =>
    ...
    if (fp.requiredDeviceTypes && deviceTypes) {
      const hasAllTypes = fp.requiredDeviceTypes.every((id) =>
        deviceTypes.includes(id)
      );
      if (!hasAllTypes) {
        return false;
      }
 ...
 */
export function getCluster(clusterId: number): ClusterDefinition | undefined {
 ...
 */
export function getVendorClusters(vendorId: number): ClusterDefinition[] {
 ...
 */
export function getVendorDevices(vendorId: number): DeviceDefinition[] {
  return allDevices.filter(
    (device) => device.fingerprint.vendorId === vendorId
  );
}
 ...
 */
export function buildRegistry(): DeviceRegistry {
  return {
    version: '1.0.0',
